...
from dataset.finewebDataset import DataLoaderLite
...
# This is for distributed data parallelism - This will return true if the system has multiple GPUs
ddp = int(os.environ.get('RANK', -1)) != -1

# If ddp is true, then we need to initialize the process group
if ddp:  # For legends
    # Given a system with 2 Machines and 4 GPUs, 
    # The number of processes is equal to the number of GPUs (8)
    # Rank: Process ID for each GPU (0 - 7). Reason: There are 8 GPUs (2 * 4) in total 
    # Local Rank: Process ID for a GPU within the node (0 - 3)
    # World Size: Number of processes (8)
...
if master_process: print("DDP: ", ddp)
...
for i in range(max_steps):
    ...
    if ddp:
        dist.all_reduce(loss_accum, op=dist.ReduceOp.AVG) # Average the loss of all GPUs
    # Gradient global clipping: Why is this used? 
    # Because the gradients can be very large and can cause overflow
